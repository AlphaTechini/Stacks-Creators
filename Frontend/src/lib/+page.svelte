  ...
  {:else if nft}
    ...
    <div>
      {#if isOwner && !nft.onChainListing}
        <!-- <ListNFT {tokenId} /> -->
        <p>You own this NFT. Component to list it for sale would go here.</p>
      {:else if !isOwner && nft.onChainListing}
        <!-- <BuyNFT {tokenId} price={nft.onChainListing.price} /> -->
        <p>This NFT is for sale. Component to buy it would go here.</p>
      {/if}
    </div>
  {/if}
</section>